-- Thomas V2 - Test Data with Real UUIDs from your created users
-- Based on the profiles export you provided

-- =============================================
-- TEST DATA WITH REAL USER UUIDs
-- =============================================

DO $$
DECLARE
  thomas_uuid UUID := 'cdc57525-43cb-4c7e-b978-6a41a4da0c0d'; -- thomas.test@gmail.com
  marie_uuid UUID := '548e3ea0-4e4d-4f96-ae82-9f4d9c1bd4e4';  -- marie.martin@ferme.fr
  pierre_uuid UUID := 'e3dbe113-2a45-4b6e-a28c-27b491d51f4c'; -- pierre.durand@conseil.fr
BEGIN

  RAISE NOTICE 'Creating test data with real UUIDs:';
  RAISE NOTICE 'Thomas: %', thomas_uuid;
  RAISE NOTICE 'Marie: %', marie_uuid;
  RAISE NOTICE 'Pierre: %', pierre_uuid;
...
-- Verify all data was created with real UUIDs
SELECT 'Farms created' as info, COUNT(*) as count FROM public.farms;
SELECT 'Farm Members created' as info, COUNT(*) as count FROM public.farm_members;
...
SELECT 'Invitations created' as info, COUNT(*) as count FROM public.farm_invitations;
