...
-- Insert default notification for task reminders
-- Only for farms where the owner_id exists in auth.users
...
SELECT 
  ...
  'Rappel tâches quotidiennes' as title,
  'N''oubliez pas d''ajouter vos tâches réalisées via le chat Thomas !' as message,
  '18:00:00' as reminder_time,
  ARRAY[1,2,3,4,5] as selected_days, -- Monday to Friday
  'task_reminder' as notification_type,
  '{"is_default": true, "auto_created": true}' as metadata
FROM public.farms f
