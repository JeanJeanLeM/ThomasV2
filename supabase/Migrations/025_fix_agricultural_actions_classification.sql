-- Migration 025: Fix Agricultural Actions Classification
-- Date: 07/01/2026
-- Context: L'IA classifie "J'ai passé la herse" comme help au lieu de task_done
-- Fix: Ajouter section spécifique pour actions agricoles avec outils

-- Désactiver l'ancien prompt
UPDATE chat_prompts 
SET is_active = false 
WHERE name = 'thomas_agent_system' AND version = '2.2';

-- Insérer le nouveau prompt version 2.3 avec exemples agricoles
INSERT INTO chat_prompts (name, version, content, is_active, created_at, examples, metadata)
VALUES (
  'thomas_agent_system',
  '2.3',
...
- **CLASSIFIER CORRECTEMENT les actions agricoles avec outils comme task_done**',
  ...
  '[
    {
      "input": "J''ai passé la herse étrie pendant 2 heures",
      "expected_action": "task_done",
      "explanation": "Verbe agricole + outil + durée = action réalisée"
    },
    {
      "input": "Comment utiliser la herse étrie ?",
      "expected_action": "help", 
      "explanation": "Question = demande d''aide"
    },
    {
      "input": "J''ai utilisé le tracteur pour labourer",
      "expected_action": "task_done",
      "explanation": "Action agricole avec outil"
    },
    {
      "input": "J''ai observé des pucerons sur les tomates",
      "expected_action": "observation",
      "explanation": "Constat avec problème spécifique"
    }
  ]',
  '{"version": "2.3", "changes": ["Added agricultural actions with tools classification", "Enhanced examples for task_done vs help discrimination", "Added comprehensive list of agricultural tools and verbs"], "fixes": ["Agricultural actions misclassified as help requests"]}'
);
